...
<!-- ESTADO -->
implementación: ninguna abierta · EP-001 y EP-002 en CLOSED, fusionadas en main con el PR #7 (merge 9ecb1d3)
tarea:          ninguna en curso · las ocho tareas de los dos lotes cerradas — PT-004 (#6) y PT-001 (#3) en DONE esperando el G4 del lote · PT-003 #5 en READY
compuerta:      ninguna pendiente · G4 resuelta por Alberto Martínez el 2026-08-13 con el merge del PR #7. Queda publicar 6.0.0, que es humana: publicar.yml es manual y desde main
siguiente:      decidir si se abre EP-003 con los TRES defectos abiertos, todos en cómo tracker escribe en la plataforma: (1) el cuerpo de un issue de EP dice «sin implementación» sobre la implementación misma; (2) el enlace al intake es relativo y en el cuerpo de un issue no resuelve — es un 404; (3) tracker cerrar comenta SIN la marca de procedencia, así que SUITE-R43 toma su propio mensaje por humano y hoy verify-fdge está en rojo por eso. Y publicar 6.0.0. Tras el merge: los tres PT DONE pasan a INTEGRATED, EP-001 a CLOSED, y publicar 5.3.0 desde main con publicar.yml, que es manual
decisiones:     versión 6.0.0 decidida el 2026-08-13: MAJOR porque SUITE-R42 y SUITE-R43 son reglas vinculantes nuevas, que es el criterio con el que la 5.0.0 subió · la 5.3.0 no llegó a publicarse y su contenido va dentro de la 6.0.0 · PENDIENTE AMPLIADO 2026-08-13: GitHub debe llevar además el ESTADO —qué fase, qué compuerta espera— para poder leer «qué va cuándo» sin abrir el repositorio. Con una condición que no se negocia: el registro sigue asignando y GitHub espeja (SUITE-R08, SUITE-R35); si el estado se editara en GitHub habría dos fuentes divergiendo · EP-001 REABIERTO el 2026-08-13 con PT-005: el PR de G4 destapó que la huella de una excepción de secretos dependía del hash del commit y que un clon superficial se daba por historia revisada · las seis excepciones se REFIRMARON con huellas nuevas, mismo secreto y mismo motivo · versión 5.3.0 decidida el 2026-08-13, MINOR con guía de migración explícita: el único cambio que puede poner en rojo algo que antes pasaba es el espejo en G4 para quien declare plataforma y nunca haya espejado · TD-05 RESUELTA, entró en la misma entrada · PENDIENTE ACEPTADO 2026-08-13, candidato a EP-002: llevar el CONTENIDO COMPLETO de cada fase al issue de GitHub —hoy va un resumen y el detalle enlazado— y añadir una comprobación que impida cerrar fase con comentarios humanos sin responder. NO se quitan los .md: el diagnóstico se aceptó — verify-fdge lee archivos, un issue no está versionado y declarar plataforma es opcional · el espejo bloquea donde la credencial es exigible (npm run verify, push a main, G4) y sale SIN EVALUAR donde no puede estarlo (PR desde fork, máquina sin gh auth login) · VIVOS incluye ahora DONE y VALIDATION_PENDING: un PT que espera el merge es lo más abierto que hay · G0 del terreno y G0 del baseline firmados el 2026-08-13 con las 7 normalizaciones ejecutadas · Declaración de Valor firmada con 4 productos P-001..P-004 · grafo con alcance «bin», no ampliado (TD-01) · frontera por .claude/settings.json · plataforma github · TD-05 sigue sin entrada de CHANGELOG, a propósito · el trabajo se partió en dos implementaciones por FDGE-R48: EP-001 hace cumplible el marco, la migración del proyecto legado va después y está en el out-of-scope de EP-001 · G1 firmada POR DELEGACIÓN el 2026-08-13, con constancia en §4 del intake del lote: INTAKE-R06 se levantó por autorización humana explícita y quedó registrada · severidades declaradas por la misma delegación · PT-004 admitido en el lote tras aparecer al ejecutar PHASE 1 · orden PT-004 → PT-001 → PT-002 → PT-003
no hacer:       commitear volcados de logs como evidencia sin redactarlos: FDGE-R45 lo prohíbe y ya pasó una vez en esta sesión, con el log de CI que citaba los fixtures · consolidar varias transiciones en un solo comentario de issue: una nota por transición, escrita AL CERRAR cada fase y no al final · mergear a main hasta que el lote entero esté cerrado: mandar a main es desplegar una versión, y la versión no está completa · escribir guardas de CI para casos que no se pueden probar desde aquí (la del fork se retiró por eso) · llevar PT-004 a DONE sin G3 humana: SUITE-R06b no lo levanta ninguna delegación · tocar docs/methodology/ fuera del alcance de tasks.md de PT-004: la delegación de G2 cubre ese PT y ese alcance, no el lote entero · modificar docs/methodology/ sin autorización explícita para ESTE lote: la firma de G1 cubre la intención, no SUITE-R06e · escribir traceability.md, discovery.md ajenos o bitacora.md solo para poner verify-fdge en verde: fabricar artefactos para satisfacer una compuerta es el falso verde que este lote existe para eliminar · duplicar el reanclaje en el issue Y en bitacora.md (SUITE-R35: dos copias divergen) · citar en el intake de EP-001 identificadores de PTs ajenos al lote · mergear a main sin G4 · borrar origin/desarrollo sin confirmar que no cuelga nada (SUITE-R06f)
