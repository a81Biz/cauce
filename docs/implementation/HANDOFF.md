...
<!-- ESTADO -->
implementación: EP-025 (#326) CLOSED · 9 tareas: 8 en estado terminal y 1 aplazada (PT-172). Version 13.4.0 escrita y alineada en los 21 documentos. LA BATERIA ESTA SELLADA Y MEDIDA: completa 2055 s / 1916 casos, sellada 346 s / 119 casos, -83%. Cuatro bloques SELLADOS (8, 9, 10, 11) y el 13 abierto, que es el que prueba la maquinaria del sello. EP-024 (#306) CLOSED y en main con 13.3.0.
tarea:          ninguna abierta. EP-025 CLOSED y sus 8 tareas en PHASE 8; falta G4 y el PR a main. El PR #346 esta abierto a trabajo.
compuerta:      G4 de EP-025 PENDIENTE. Es el merge de trabajo a main, humano por definicion (SUITE-R06a). npm publish SIGUE RESERVADO y una autorizacion posterior que no lo mencione NO lo deroga.
siguiente:      1) PR de la rama del lote a trabajo · 2) G4: merge de trabajo a main · 3) tag v13.4.0 DESPUES del merge (SUITE-R06a) · 4) cerrar el issue #326 con «tracker cerrar --aplicar» cuando el estado terminal este en la rama por defecto (SUITE-R46) · 5) EP-026, que recoge PT-179 (S1), PT-181 y PT-187.
decisiones:     1) El bloque agrupa por MAJOR y su MAJOR sale del COMMIT que introdujo la seccion: retroactivo por construccion, sin declarar nada, y cada destino lo tiene en su propio git. 2) Un bloque se certifica por haber PASADO, no por no haber cambiado: el sello guarda su VEREDICTO y sellar-bloques se niega sin --verde. 3) Reabrir NO es volver a correr: un sello que no casa exige una corrida COMPLETA nueva. 4) verificacion.yml salta lo sellado (corre en cada push); publicar.yml corre la COMPLETA a proposito (workflow_dispatch, solo desde main). 5) La cuenta de herramientas y de pasos NO se escribe a mano: se quita, no se corrige.
no hacer:       -19) CORRER «npm run verify» ENTERO, no los verificadores por separado: verify:secretos solo esta ahi, y por saltarmelo la CI del PR bloqueo con ocho falsos positivos (PT-190). SUITE-R01 existe para esto. -18) UN CASO PUEDE FIJAR EL CERO DE LO PROHIBIDO, NUNCA EL NUMERO DE LO CORRECTO. Un caso con «^9 pasos$» fallo al anadir un paso legitimo: castigaba la mejora. Tercera vez en dos lotes. -17) EL ARNES PUDO ESCRIBIR EN EL REPOSITORIO REAL: arreglado en PT-188 con dos puertas, pero antes de correrlo conviene mirar «git log -1» y el numero de allocations. -16) El sello del HANDOFF va al final, en su propio commit. -15) NO trabajar sobre «trabajo»: la rama efimera del lote es obligatoria (FDGE-R19), y en ESTE lote se salto. -14) «node -e» pone los argumentos en argv[1], no en argv[2], y se come el «--»: dos casos midieron el mismo estado por eso. -13) Las banderas son --epica y --severidad. -12) EL ORDEN DE LOS DERIVADOS: eventos ANTES que matriz, los dos DESPUES de la ultima edicion. -11) NO editar tools/ con la bateria corriendo: invalido TRES corridas de 34 min en esta sesion.
lo que este lote descubrio: el SELECTOR de bloques fallaba en SILENCIO y su silencio significa «no acotes», asi que la bateria habria corrido entera para siempre sin que nada lo dijera · dos herramientas nuevas SIN DECLARAR, cazadas por audit y no por mi · SUITE-R62 reabierto: «sellar:bloques» entro en la CI y no en «npm run verify», el hueco exacto de PT-151 · CLAUDE.md declaraba 18 herramientas en un sitio y 15 en otro, y hay 20 · el trabajo del lote estaba ocurriendo SOBRE «trabajo» y un commit llego alli sin PR, en el lote que existe para impedir saltarse el marco.
hallazgo sin tarea: NINGUNO. EP-026 tiene PT-179 (S1), PT-181 y PT-187. Declarados con vuelta al 2026-09-30: la forma de los bloques (PT-172) · el paso de sincronizacion que PHASE 9 no declara (PT-186) · el ciclo de dos viajes al tablero (PT-186) · la severidad de PT-179/181 (PT-183).
pregunta abierta: si el trazo PT -> caso debe derivarse del grafo, como pidio el firmante. Existe en el nombrado —las secciones nombran su PT— y NADIE lo deriva. Y graphify, que el marco exige (FND-R14, FDGE-R43), tiene graphify-out/graph.json VACIO: «{}», 3 bytes, mientras GRAPH_REPORT.md declara 1028 nodos.
lo que sigue sin resolverse: PT-025 (#35) DEFERRED · PT-172 DEFERRED en este lote · 21 allocations sin suite_version · 4 PT historicos sin lote · el slug de PT-155 sigue divergiendo, ahora NOMBRADO · v9.0.0, v10.0.0 y v5.2.0 tienen tag y no estan en npm · 4.13.0, 5.0.0 y 5.1.0 estan en npm y no en el CHANGELOG.
actualizado:    2026-08-28 · PT-190 en PHASE 8 Persistencia
<!-- /ESTADO -->
