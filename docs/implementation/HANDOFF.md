...
<!-- ESTADO -->
implementación: EP-025 (#326) REABIERTA y con 12 tareas: 10 en DONE, 1 aplazada (PT-172) y ninguna abierta. NO puede cerrarse hasta despues del merge: DONE no es terminal, INTEGRATED si (SUITE-R46). Version 13.4.0. LA BATERIA SE CERTIFICA POR UNA CORRIDA, NO POR UNA BANDERA (PT-191): completa 1923 casos / 27m24s, y con SU RECIBO se sellaron los 4 bloques; acotada despues 126 casos / 4m23s. EP-024 (#306) CLOSED y en main con 13.3.0.
tarea:          ninguna abierta. PT-190, PT-191 y PT-193 en PHASE 8 DONE. El PR #346 esta abierto a trabajo.
...
siguiente:      1) merge del PR a trabajo · 2) «tracker integrar» DONE->INTEGRATED · 3) «tracker cerrar --aplicar» los issues (SUITE-R46) · 4) «tracker cierre EP-025 --aplicar» · 5) G4: merge de trabajo a main · 6) tag v13.4.0 DESPUES del merge (SUITE-R06a) · 7) EP-026, que recoge PT-179 (S1), PT-181, PT-187, PT-192 y PT-194.
decisiones:     1) El bloque agrupa por MAJOR y su MAJOR sale del COMMIT que introdujo la seccion. 2) UN BLOQUE SE CERTIFICA POR HABER PASADO, y desde PT-191 eso se comprueba: «selftest.sh --todo» deja CORRIDA.json con veredicto, recuento y HUELLA DEL ARNES, y «sellar-bloques --verde» se niega sin recibo, con recibo ilegible, con recibo en rojo o con recibo de otra bateria — y dice CUAL de las cuatro. 3) EL RECIBO NO SE VERSIONA: si viajara, cualquier clon podria sellar sin correr nada. El SELLO si: es la decision. 4) Reabrir NO es volver a correr. 5) verificacion.yml salta lo sellado; publicar.yml corre la COMPLETA a proposito. 6) Los literales de fixture se ENSAMBLAN en mitades (PT-193, precedente PT-015): el archivo generado no cambia, el FUENTE deja de llevarlos.
no hacer:       -24) LOS DERIVADOS VAN DESPUES DE LA ULTIMA EDICION, TAMBIEN SI ES UN COMENTARIO: corri «inventario --aplicar» antes de anadir tres comentarios y la cifra de lineas quedo obsoleta — 1922 verdes, 1 rojo, 28 minutos tirados. -23) NO CITAR EL VALOR DE UN FIXTURE EN LA DOCUMENTACION: escribi los documentos de la tarea que SACA el literal del repositorio citando el literal, y FDGE-R45 me cazo en cuatro archivos. -22) EL ANCLA DE UNA INSERCION CASA LA PRIMERA APARICION: meti el bloque del recibo en mitad de la seccion C, donde TOTAL y FAILED no son definitivos; habria escrito un recibo A MITAD DE CORRIDA. Usa rindex y comprueba que no queda ningun caso detras. -21) NUNCA RUTAS RELATIVAS AL FINAL DEL ARNES: hace «cd $WORK» en el shell PRINCIPAL (1159, 3019), asi que ${BASH_SOURCE[0]} no resuelve. RAIZ y SUITE son absolutas desde la 17. Por esto una corrida de 1923 casos termino SIN RECIBO y nada lo dijo. -20) EL HEREDOC DEL SHELL SE COME LOS BACKSLASH: para codigo con \n o continuaciones, escribe el script a un archivo y ejecutalo. Rompio el printf del recibo dos veces. -19) CORRER «npm run verify» ENTERO, no los verificadores por separado. SUITE-R01 existe para esto. -18) UN CASO PUEDE FIJAR EL CERO DE LO PROHIBIDO, NUNCA EL NUMERO DE LO CORRECTO. Cuarta y quinta aparicion en PT-191: «tail -4» y «tail -40» sobre el propio fuente. Ya tiene tarea: PT-192. -17) EL ARNES PUDO ESCRIBIR EN EL REPOSITORIO REAL: arreglado en PT-188, pero mira «git log -1» y el numero de allocations antes de correr. -16) El sello del HANDOFF va al final, en su propio commit. -15) NO trabajar sobre «trabajo»: la rama efimera del lote es obligatoria (FDGE-R19). -14) «node -e» pone los argumentos en argv[1]. -13) Las banderas son --epica y --severidad. -12) eventos ANTES que matriz. -11) NO editar tools/ con la bateria corriendo.
lo que este lote descubrio: el sello se estampaba con una BANDERA y nadie comprobaba que la corrida ocurriera — el caso que lo destapo es real, PT-190 reabrio el bloque 8 y la corrida que lo devolvio al verde fue la ACOTADA: sellar ahi habria certificado 16 secciones que ese dia NO corrieron · CINCO defectos en el arreglo del sello, TRES del codigo que la sesion anterior dejo escrito sin correr y DOS mios, y los cinco producian SILENCIO en vez de rojo · commitear el fixture de PT-190 metio una contrasena sintetica en la HISTORIA, donde «cauce:senuelos» no llega · el mismo valor estaba DOS veces, no una, y con la segunda en pie el AC era INALCANZABLE.
hallazgo sin tarea: NINGUNO. EP-026 tiene PT-179 (S1), PT-181, PT-187, PT-192 y PT-194. Los tres nuevos citan la parada de PT-191 que los produjo (FDGE-R55), con su explicacion en changes/PT-191-.../paradas/ y publicada en el issue #348.
...
actualizado:    2026-08-28 · PT-193 en PHASE 8 · EP-025 lista para el merge
<!-- /ESTADO -->
