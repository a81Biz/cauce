...
tarea:          PT-005 · PHASE 6 completa · VALIDATION_PENDING · issue #8 — las otras cuatro de EP-001 cerradas — PT-004 (#6) y PT-001 (#3) en DONE esperando el G4 del lote · PT-003 #5 en READY
compuerta:      G3 de PT-005 pendiente · Alberto Martínez · y después el G4 del lote, humano sin excepción (EXEC-R04, SUITE-R06a)
siguiente:      resolver G3 de PT-005 leyendo evidence/PT-005/, y confirmar en el PR #7 que el paso de secretos pasa —es el AC-06, que solo se comprueba ahí—. Después: revisar el PR y apretar el merge. Tras el merge: los tres PT DONE pasan a INTEGRATED, EP-001 a CLOSED, y publicar 5.3.0 desde main con publicar.yml, que es manual
decisiones:     EP-001 REABIERTO el 2026-08-13 con PT-005: el PR de G4 destapó que la huella de una excepción de secretos dependía del hash del commit y que un clon superficial se daba por historia revisada · las seis excepciones se REFIRMARON con huellas nuevas, mismo secreto y mismo motivo · versión 5.3.0 decidida el 2026-08-13, MINOR con guía de migración explícita: el único cambio que puede poner en rojo algo que antes pasaba es el espejo en G4 para quien declare plataforma y nunca haya espejado · TD-05 RESUELTA, entró en la misma entrada · PENDIENTE ACEPTADO 2026-08-13, candidato a EP-002: llevar el CONTENIDO COMPLETO de cada fase al issue de GitHub —hoy va un resumen y el detalle enlazado— y añadir una comprobación que impida cerrar fase con comentarios humanos sin responder. NO se quitan los .md: el diagnóstico se aceptó — verify-fdge lee archivos, un issue no está versionado y declarar plataforma es opcional · el espejo bloquea donde la credencial es exigible (npm run verify, push a main, G4) y sale SIN EVALUAR donde no puede estarlo (PR desde fork, máquina sin gh auth login) · VIVOS incluye ahora DONE y VALIDATION_PENDING: un PT que espera el merge es lo más abierto que hay · G0 del terreno y G0 del baseline firmados el 2026-08-13 con las 7 normalizaciones ejecutadas · Declaración de Valor firmada con 4 productos P-001..P-004 · grafo con alcance «bin», no ampliado (TD-01) · frontera por .claude/settings.json · plataforma github · TD-05 sigue sin entrada de CHANGELOG, a propósito · el trabajo se partió en dos implementaciones por FDGE-R48: EP-001 hace cumplible el marco, la migración del proyecto legado va después y está en el out-of-scope de EP-001 · G1 firmada POR DELEGACIÓN el 2026-08-13, con constancia en §4 del intake del lote: INTAKE-R06 se levantó por autorización humana explícita y quedó registrada · severidades declaradas por la misma delegación · PT-004 admitido en el lote tras aparecer al ejecutar PHASE 1 · orden PT-004 → PT-001 → PT-002 → PT-003
