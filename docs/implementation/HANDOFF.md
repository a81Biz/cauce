...
<!-- ESTADO -->
implementación: EP-022 (#286) CLOSED. Las ocho INTEGRATED: PT-144, PT-145, PT-146, PT-147, PT-148, PT-149, PT-150 y PT-156. Su criterio de exito se cumple y esta PROBADO. G4 resuelta: PR #314 fusionado a main el 2026-08-26. Y EP-024 (#306) con VEINTE tareas, todas APLAZADAS con condicion de reentrada, fecha y dueno (LEX-R34).
tarea:          NINGUNA en curso, y ningun lote abierto con trabajo empezado. El ultimo termino de punta a punta: #315 y #317 fusionados, main = trabajo (4165bfc), nueve issues cerrados, espejo 22/22, sin ramas sueltas.
compuerta:      TODAS resueltas. G4 ejecutada por autorizacion expresa del firmante, con alcance y limites en SESSION_LOG del 2026-08-26.
siguiente:      DECISION DEL FIRMANTE, no hay trabajo en curso que retomar. Tres puertas: (a) descomponer EP-024 —veinte hallazgos aplazados, y PT-168 es S1 y toca lo que el marco AFIRMA verificar—; (b) descomponer EP-023 (DICTAMEN), firmada y sin tareas; (c) publicar la 13.2.0 con publicar.yml en manual desde main, que SIGUE RESERVADO al firmante.
decisiones:     1) VoBo para EP-022 entero incluido G4, borrado de ramas y cierre del BUG, con constancia y limites escritos. 2) npm publish SIGUE RESERVADO, y una autorizacion amplia posterior NO lo deroga. 3) PT-150 y PT-156 entraron al alcance por decision del firmante. 4) La poda de la bateria va a OTRA epica: PT-169. 5) Version 13.2.0 MINOR, confirmado y no supuesto.
no hacer:       -9) sellar el HANDOFF antes de «integrar»: integrar TOCA los intakes, y SUITE-R34 mide trabajo en changes/ contra el sello. El HANDOFF va SIEMPRE al final. -8) escribir una constancia de autorizacion con un encabezado que no diga G4, VoBo o «autorizad»: EXEC-R04 no la reconoce y el merge sale como NO autorizado teniendola. -7) dar por roja una bateria por el exit code: un `grep -c` final devuelve 1 cuando la cuenta es CERO. Se mira el ARCHIVO. -6) aplazar y no volver a medir. -5) publicar una parada con «declara»: deja el hallazgo HUERFANO; el enlace es «--desenlace abre --abre PT-NNN». -4) avanzar la tarea N+1 con el PR de la N abierto. -3) marcar un AC CUMPLIDO antes de tener el exit code de su prueba. -2) fiarse del exit code de una tuberia con grep. -1) derivar MATRIZ.md antes de escribir HISTORY.log. 0) DOS baterias a la vez. 1) npm publish. 2) escribir REGISTRY.json a mano. 3) editar selftest.sh NI NINGUNA HERRAMIENTA QUE LOS CASOS INVOQUEN mientras la bateria corre: audit la ejecutan cincuenta casos, y editarla a mitad de vuelo produjo dos rojos que eran una FOTO MOVIDA, no una regresion. Se puede tocar documentos, intakes y evidencia; nada que un caso vaya a ejecutar. 4) sustituir texto contra un ancla sin comprobar que es UNICA. 5) commitear el HANDOFF antes que el trabajo. 6) suponer que «npm run verify» es lo que corre CI: CI usa --all y verify NO. 7) poner prosa donde FDGE-R23 espera una ruta. 8) escribir traceability con unos AC distintos de los del intake. 9) buscar la carpeta de un intake por prefijo: se DERIVA del slug. 10) declarar un AC «estructural» sin escenario.
lo que este lote descubrio: DIECISEIS sitios, no trece · E5 era FALSO en sus dos mitades y solo lo dijo EJECUTARLO: de las cuatro herramientas solo verify-suite veia un componente nuevo · SEIS fijaciones impedian el alta, CUATRO en FAMILIAS · el alta real son SEIS pasos y uno no lo decia nadie (prefijos() sale de FAMILIAS) · FIDE faltaba del mapa de fases de CORE · audit da por cubierta una fase si el NUMERO aparece en cualquier sitio del documento · tres casos de PT-147 solo podian pasar MIENTRAS existia el defecto · tracker aplazar escribia dos campos que nadie lee y dejaba el intake contradiciendo al registro · EXEC-R04 rechazo una constancia REAL por una palabra del encabezado.
...
actualizado:    2026-08-26 · PT-151 en PHASE 8 Persistencia
<!-- /ESTADO -->
