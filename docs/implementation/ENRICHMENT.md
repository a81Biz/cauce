# ENRICHMENT — índice de features

Índice, no contenido (`LEX-R12`). Una línea por PT; el enriquecimiento vive en
`changes/PT-XXX-slug/`.

> **DERIVADO del registro.** No se edita a mano: `tracker indices --aplicar`
> lo regenera. Editarlo aquí se pierde en la siguiente regeneración, y editarlo a mano es
> lo que dejó catorce filas pegadas en una línea en el índice de refactors.

| Id | Tipo | Sev | Estado | Lote | Título |
|:---|:---|:---|:---|:---|:---|
| PT-007 | FEATURE | S3 | INTEGRATED | EP-002 | El issue declara en qué fase está y qué compuerta espera, derivado del registro |
| PT-008 | FEATURE | S2 | INTEGRATED | EP-002 | No se cierra fase con comentarios humanos sin responder |
| PT-030 | FEATURE | S1 | INTEGRATED | EP-007 | tracker siguiente: que toca y como se cierra, derivado del tablero y no del criterio del agente |
| PT-032 | FEATURE | S1 | CLOSED | EP-008 | Convencion o agente propio de cauce que ate al agente al tablero sin que haya que recordarselo |
| PT-033 | FEATURE | S1 | INTEGRATED | EP-008 | La convencion: CORE.md abre con el estado del tablero y ninguna fase avanza sin haberlo consultado |
| PT-034 | FEATURE | S1 | INTEGRATED | EP-008 | Un agente propio de cauce cuyo punto de entrada es el tablero: no una regla que se pueda saltar |
| PT-037 | FEATURE | S1 | INTEGRATED | EP-010 | Enumerar todos los casos de uso y, por cada uno, su ruta exacta hasta el final |
| PT-038 | FEATURE | S1 | INTEGRATED | EP-010 | Escribir el manual: de cero al primer PT cerrado siguiendo un solo documento |
| PT-039 | FEATURE | S1 | INTEGRATED | EP-011 | Distinguir una peticion de una conversacion: la regla primero, y despues el arranque |
| PT-040 | FEATURE | S1 | INTEGRATED | EP-011 | Que puede fallar, DERIVADO de los fail() reales y no escrito de memoria |
| PT-041 | FEATURE | S1 | INTEGRATED | EP-011 | cauce regla SUITE-RNN, y que el mensaje del fallo lleve a ella |
| PT-042 | FEATURE | S1 | INTEGRATED | EP-011 | El agente lee su manual al instalar y al arrancar, y sabe autorreferenciarse |
| PT-043 | FEATURE | S1 | INTEGRATED | EP-011 | Las siete decisiones de migrar un legado se CONDUCEN, no se enumeran |
| PT-069 | FEATURE | S2 | INTEGRATED | EP-017 | Los indices derivados necesitan generador |
| PT-078 | FEATURE | S1 | INTEGRATED | EP-017 | Ninguna regla queda sin clasificar: el universo son 223 y cubierta significa emitida |
| PT-104 | FEATURE | S1 | INTEGRATED | EP-019 | El tablero dice en que paso estas, cual es la regla de entrada y cual la de salida |
| PT-115 | FEATURE | S1 | DRAFT | EP-020 | PARADA entra al vocabulario y a las reglas |
| PT-116 | FEATURE | S1 | DRAFT | EP-020 | tracker parada: el comando que escribe la parada en su tarea |
| PT-117 | FEATURE | S1 | DRAFT | EP-020 | Todo desenlace cita la parada que lo produjo |
| PT-118 | FEATURE | S1 | DRAFT | EP-020 | La taxonomia de clases de evento, cerrada, en LEXICON |
| PT-119 | FEATURE | S1 | DRAFT | EP-020 | tools/matriz.mjs deriva MATRIZ.md |
| PT-128 | FEATURE | S1 | DRAFT | EP-020 | El cursor: donde estas, de donde vienes, a donde vas, y ningun nodo sin visitar |
