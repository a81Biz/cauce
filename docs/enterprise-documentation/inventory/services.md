...
|:---|---:|:---|:---|:---|
| `selftest.sh` | 8152 | 697 casos sobre un proyecto sintético con defectos inyectados | crea y borra `$WORK` | `npm run selftest` · CI |
...
| `build-core.mjs` | 572 | Compila `CORE.md` y `CORE-PTSA.md` desde las fuentes normativas | **sí** · `--check` no | `npm run core` · `core:check` · CI |
...
| `verify-patrones.mjs` | 345 | Contrato de `patrones.mjs`: cada patrón con lo que debe casar y lo que no | no | `npm run verify:patrones` · CI |
