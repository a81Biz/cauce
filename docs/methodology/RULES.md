...
| `SUITE-R44` | HARD | **Cerrar un lote no borra lo que aplazó.** Toda fila de `out-of-scope.md` que apunte a **trabajo futuro** cita el identificador que lo sostiene, y ese identificador es una `allocation` del registro —normalmente en estado `DEFERRED`— con su issue si el proyecto declara plataforma. Se reconoce que una fila apunta a trabajo futuro por una lista corta y **declarada** de expresiones: «posterior», «siguiente», «más adelante», «otra implementación», «futuro» — una heurística escondida sería peor que una explícita. Fuera de `G4` se avisa, porque aplazar **durante** el trabajo es legítimo y frecuente; en `G4` bloquea, porque es donde el lote se cierra. `DEFERRED` existía en `LEXICON` §5.1 sin que lo usara nadie: un aplazado queda **exento** de las exigencias de artefactos de un PT en curso —no tiene intake ni ha recorrido fases— y **vivo** para el espejo, de modo que su issue permanece abierto y en el tablero. Existe porque un lote de esta suite aplazó la corrección que lo había motivado y nadie la recogió en cuatro versiones: estaba escrita en tres documentos y en ninguna lista que una compuerta tocara. |
