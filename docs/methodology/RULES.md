...
| `FDGE-R52` | HARD | **Reanclaje en cada transición de fase.** Antes de cambiar de fase y antes de abrir nada nuevo, se relee el bloque `ESTADO` y se dice en una línea dónde se está. `PHASE 0` lee el estado **una vez**, al arrancar; después no hay nada que lo devuelva a la ventana, y en una sesión larga el contexto se comprime y las reglas salen. Cuesta poco precisamente porque el bloque es corto: es coherente con `SUITE-R24`, no una excepción. **Es la regla más débil del marco** — de la deriva dentro de una sesión se consigue que se note al final, no impedirla en el momento. |
